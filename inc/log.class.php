...
class PluginChecksLog extends CommonDBTM {
...
//	echo var_dump(self::prepareForAdd($input));
  ...
  /*  $Notification= new QueuedNotification();
	  $Notification->showForm($ID, $options);*/
...
//	echo $_SESSION["check_id"];

if (count(explode("/front/log.php",$_SERVER["SCRIPT_FILENAME"]))>1) {

echo "<h3>Chequeos -  <a href='check.form.php?id=".$_SESSION["check_id"]."'>". strtoupper($_SESSION["check_name"]). "</a></h3>";
											 
} 
											 
$opciones = Search::manageParams('PluginChecksLog', $_GET);

//var_dump($opciones);
										 
$data=Search::prepareDatasForSearch('PluginChecksLog',  $opciones);	 
						 			
 if ($data["search"]["sort"]==1) {
  $data["search"]["sort"]=12;
  $data["search"]["order"]="DESC";
 }
...
Search::constructSQL($data);
 ...
 // var_dump($data);
  ...
  if(!empty($data["sql"]["count"])){
 $where=explode("WHERE", $data["sql"]["count"][0]);
 //$order
 $data["sql"]["count"][0]=$where[0]." WHERE (`glpi_plugin_checks_logs`.`items_id` = '".$_SESSION["check_id"]."') and ". $where[1];
// echo  $data["sql"]["count"]."<br><br><br>";
 }

  
  if(!empty($data["sql"]["search"])){
 $where=explode("WHERE", $data["sql"]["search"]);
 //$order
 $data["sql"]["search"]=$where[0]." WHERE (`glpi_plugin_checks_logs`.`items_id` = '".$_SESSION["check_id"]."') and ". $where[1];
 //echo  $data["sql"]["search"]."<br><br><br>";
 }
 ...
 Search::showGenericSearch('PluginChecksLog', $opciones);
